...
# === ACAS-SI quality-signal selection. Defaults = precision ACAS-SI. ===
#   ACAS_SI_SIGNAL : "precision" (default) | "l2" | "contrib"   ACAS_SI_TARGET : metric target
#   ACAS_SI_MODE   : "equalize" (default) | "ceiling"           ACAS_SI_FREEZE=1 : pin alphas
#   ACAS_SI_INIT_ALPHAS : warm-start json     ACAS_SI_KP/KI/KD : PID gains
...
SI_MODE = os.environ.get('ACAS_SI_MODE', 'equalize').lower()
SI_FREEZE = os.environ.get('ACAS_SI_FREEZE', '0') == '1'
SI_INIT_JSON = os.environ.get('ACAS_SI_INIT_ALPHAS', '')
SI_KP = float(os.environ.get('ACAS_SI_KP', '1.0'))
SI_KI = float(os.environ.get('ACAS_SI_KI', '0.2'))
SI_KD = float(os.environ.get('ACAS_SI_KD', '0.15'))
...
# log all metrics every tick (quality/l2_error/skip_ratio/contrib_quality appended; old cols unchanged)
...
class SparseLlamaModel(SparseLlamaPreTrainedModel):
    ...
    def __init__(self, config: SparseLlamaConfig):
        ...
        self.post_init()
        self.pid = [BufferedPID(SI_KP, SI_KI, SI_KD, 5, 5) for _ in range(config.num_hidden_layers)]
        ...
        if SI_INIT_JSON:
            ...
            with open(SI_INIT_JSON) as _f:
                _wa = _json.load(_f)
            for i in range(config.num_hidden_layers):
                self.alphas[i] = int(_wa[i] if isinstance(_wa, list) else _wa[str(i)])
            print(f"[ACAS-SI] warm-started alphas from {SI_INIT_JSON} (mean={sum(self.alphas)/len(self.alphas):.0f})")
        if SI_FREEZE:
            print("[ACAS-SI] FREEZE mode: alphas fixed; metrics measured + logged only")
        self.step = 0

        # --- cross-chunk controller-state persistence (cycling/adaptation experiment) ---
        # Dormant unless ACAS_STATE_FILE is set; vast table runs don't set it -> no effect there.
        import os as _os, atexit
        self._state_file = _os.environ.get('ACAS_STATE_FILE', '')
        if self._state_file and _os.path.exists(self._state_file):
            import json as _json
            with open(self._state_file) as _f:
                _state = _json.load(_f)
            for i in range(min(len(_state.get('alphas', [])), config.num_hidden_layers)):
                self.alphas[i] = _state['alphas'][i]
            self.step = _state.get('step', 0)
            for i in range(min(len(_state.get('pid_integral', [])), config.num_hidden_layers)):
                if hasattr(self.pid[i], '_integral'):
                    self.pid[i]._integral = _state['pid_integral'][i]
            print(f"[STATE] Loaded state from {self._state_file}: step={self.step} alphas[:4]={self.alphas[:4]}")

        def _auto_save_state(model_ref=self):
            try:
                if model_ref._state_file:
                    import json as _json
                    state = {
                        'alphas': [model_ref.alphas[i] for i in range(model_ref.config.num_hidden_layers)],
                        'step': model_ref.step,
                        'pid_integral': [getattr(model_ref.pid[i], '_integral', 0.0) for i in range(model_ref.config.num_hidden_layers)],
                    }
                    _tmp = model_ref._state_file + '.tmp'
                    with open(_tmp, 'w') as _f:
                        _json.dump(state, _f, indent=2)
                    _os.replace(_tmp, model_ref._state_file)
                    print(f"[STATE] Saved state to {model_ref._state_file}")
            except Exception as _e:
                print(f"[STATE] Failed to save: {_e}")
        atexit.register(_auto_save_state)
    ...
    def forward(
        ...
        if any(x is not None for x in signals):
            for idx, signal in enumerate(signals):
                ...
                if SI_SIGNAL == "precision":
                    metric = signal["precision"]
                elif SI_SIGNAL == "contrib":
                ...
                if SI_MODE == "ceiling" and not SI_FREEZE:
                    error = max(0.0, error)   # protect-only: never free safe layers
                if SI_CONTROLLER == 'rl':
                    push = self.rl[idx].update(metric)
                    p_term = i_term = d_term = 0.0
                    adjustment = 0 if SI_FREEZE else int(round(push * self._alpha_unit))   # 1 unit = 10000/(hidden/2)
                    ...
                    if adjustment > 0:
                        ...
                        adjustment -= 0.5
                    if SI_FREEZE:
                        adjustment = 0   # pin alphas (baseline / frozen measurement)
