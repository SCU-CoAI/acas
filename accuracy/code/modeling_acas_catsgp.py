...
PRED_AUDIT_PROB = 0.005          # probability of doing an audit step
TARGET_WRONG_SKIP = 0.05        # target wrong-skip share

# --- contribution-metric signal (two-knob CATS-GP) ----------------------------------
# Base: methods/gr+t/auto (the paper's Fig-6 grid file — Grasp sign-bit predictor whose thresholds
# transfer DIRECTLY to the C++ Q1 kernel). Change ONLY the per-knob PID quality signal, nothing else.
...
class AdaptiveActivationSparsityController:
    ...
    def update(self, activated_gate, do_quality_check: bool = True, up_output=None, down_proj=None, residual=None):
        if self.threshold is None:
            activated_abs_float = activated_gate.abs().float()
            self.threshold = torch.quantile(activated_abs_float, 0.20).item()
            return self.threshold, 0.20, 1.0
...
class LlamaMLP(nn.Module):
    ...
    def _load_s_values(self):
        ...
        # A silent fallback to defaults would change the predictor, so print which source is used.
    ...
    def forward(self, x, residual=None):
        if x.size(1) == 1 and self.enable_adaptive_sparsity:
            ...
            # Audit decision for predictor controller
            audit = self._pred_ctrl.should_audit()

            if audit:
                # Full compute for teacher and metrics.
                # up_output/down_proj/residual let each knob measure its STANDALONE
                # output-contribution drift when ACAS_CATSGR_SIGNAL='contrib'.
                ...
                activation_threshold, activation_sparsity, activation_quality = self.activation_sparsity_controller.update(
                    activated_full, do_quality_check=True,
