...
""" PyTorch LLaMA model — GRASP + MIMD interval scheduling."""
import math
import warnings
import atexit
import os as _os
...
NUM_LAYERS = 32
MAX_ALPHA = 15000   # aligned with the fixed-cadence Grasp file (was 10300, too tight given INIT=10000)
MIN_ALPHA = 5000
...
# MIMD per-layer adaptive injection interval
MIMD_INCREASE = 1.5
MIMD_DECREASE = 0.5
...
# Identical signal swap to the PID-contrib Grasp file (modeling_acas_grasp.py): change
# ONLY the per-layer quality SIGNAL driving the alpha controller; the MIMD adaptive
# check-interval machinery (layer_update + _mimd_interval) is left untouched and now adapts on the
# same signal.
#   ACAS_GRASP_SIGNAL : "precision" (default, TP/(TP+FP)) | "l2" (local drift) | "contrib"
#                       ("contrib" = ||y_dense - y_sparse|| at the MLP OUTPUT, normalized by ||residual||)
#   ACAS_GRASP_TARGET : metric target (default = TARGET_P for precision, else 0.95)
#   ACAS_GRASP_MODE   : "equalize" (default) | "ceiling" (protect-only)   ACAS_GRASP_FREEZE=1 : pin alphas
#   ACAS_GRASP_TARGET_SLOPE : depth-weighted target (0 -> flat); mean target stays ~GRASP_TARGET
import os
...
# GRASP proportion list (per-layer coefficients for outlier/big/middle/small buckets)
proportion_list = [
...
class SparseLlamaModel(SparseLlamaPreTrainedModel):
    ...
    def __init__(self, config: SparseLlamaConfig):
        ...
        self.alphas = [INIT_ALPHA] * config.num_hidden_layers
        self.step = 0
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            # New sequence — save summary of previous
        ...
        if any(x is not None for x in signals):
            for idx, signal in enumerate(signals):
                ...
                if GRASP_SIGNAL == "contrib":
                    metric = signal["contrib_quality"]   # output-contribution drift (residual-relative)
                elif GRASP_SIGNAL == "l2":
                    metric = signal["quality"]           # local post-activation L2 drift
                else:
                    metric = signal["precision"]         # Grasp precision (default)
                # depth-weighted target: looser (lower) early, stricter (higher) late (SLOPE=0 -> flat)
                ...
                # Per-layer MIMD interval update — adapt the check cadence on the SAME metric/target
                # the controller optimizes (contrib drift in contrib mode), keeping the two consistent.
                abs_error = abs(metric - target_l)
            ...
            if self._track_enabled:
                ...
                for idx2 in range(self.config.num_hidden_layers):
                    ...
                    record[f'precision_L{idx2}'] = signals[idx2]["precision"] if signals[idx2] is not None else -1
