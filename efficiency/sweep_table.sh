...
N="${N:-6}"; GAP="${GAP:-8}"; NG="${NG:-2048}"; ONLY="${ONLY:-SI GR CATS CATSGP}"   # method filter, e.g. ONLY="SI CATS"
EFF="$ROOT/efficiency"
...
do_reps "SI precPID" "$(relu "$SID" "$SB" "$PROSPARSE" precision pid fixed 0.995 "$SA/precision_0.995.txt")"
...
do_reps "GR precPID" "$(relu "$GD" "$GB" "$PROSPARSE" precision pid fixed 0.997 "$GA/precision_0.997.txt")"
...
do_reps "CATS L2PID" "$(silu "$CD" "$CB" "$LLAMA" l2 pid fixed 0.80 "$CC/l2_0.80.txt")"
...
do_reps "CATSGP L2PID" "$(silu "$PD" "$PB" "$LLAMA" l2 pid fixed 0.80 "$PC/l2_0.80_g0.05.txt" 0.05)"
