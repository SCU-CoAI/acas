...
// Async confusion matrix processing system
...
private:
    ...
    int n_layers;
    int* alpha;
    const int ALPHA_MIN = 9500;
    const int ALPHA_MAX = 15000;
    ...
    // seed = layer index). The RL push (decision-boundary units) becomes an alpha step via
...
public:
    AsyncSparseController(BufferedPID* pids, int n_layers, int* alpha);
