...
// and the async controller's precision = TP/(TP+FP) recovers contrib exactly.
...
struct ggml_tensor * ggml_eval_async(
    ...
    // Store layer_id and step in extra field as packed int64
...
void ggml_gate_q1(
    ...
    memcpyD2H(tmp, gate->data, sizeof(ggml_half) * gate->ne[0] * gate->ne[1]);
    ...
    memcpyH2D(dst->data, dst_tmp, sizeof(int) * dst->ne[0] * dst->ne[1]);
...
void transpose_mat(struct ggml_tensor * down, struct ggml_tensor * dst) {
    ...
    memcpyD2H(tmp, down->data, down->ne[0] * down->ne[1] * sizeof(ggml_half));

    ggml_half * dst_tmp = (ggml_half *) malloc (sizeof(ggml_half) * dst->ne[0] * dst->ne[1]);

    transpose_tile(tmp, dst_tmp, down->ne[0], down->ne[1]);

    memcpyH2D(dst->data, dst_tmp, dst->ne[0] * dst->ne[1] * sizeof(ggml_half));
    free(tmp);
    free(dst_tmp);
}
