...
AsyncSparseController::AsyncSparseController(BufferedPID* pids, int n_layers, int* alpha)
    ...
    // default 'pid' keeps the BufferedPID path byte-identical.
    ...
    if (const char * c = getenv("ACAS_CADENCE")) {
        if (std::strcmp(c, "mimd") == 0 && !g_mimd) {
            ...
            float nmin = en ? atof(en) : 50.0f;   // Nmin=50 (Alg. 1 default); override via ACAS_NMIN
        ...
        while (!cm_queue.empty()) {
            ...
            if (total > 0) {
                ...
                if (use_rl) {
                    ...
                    adj_rounded = (int) lroundf(push * ALPHA_UNIT);
                } else {
                    error = calculate_error(precision, TARGET_P);
                    pids[cm.layer_id].add_sample(error);
                    adj_rounded = (int) roundf(pids[cm.layer_id].get_update());
                }

                // Update alpha value
                alpha[cm.layer_id] += adj_rounded;
                ...
                if (acas_log) LOG("Layer %d: precision=%.4f, error=%.4f, alpha=%d->%d\n", 
                   cm.layer_id, precision, error, alpha[cm.layer_id] - adj_rounded, alpha[cm.layer_id]);
            }
...
void init_async_controller(BufferedPID* pids, int n_layers, int* alpha) {
