...
static __global__ void eval_impl(const float * x, float * dst) {
...
static __global__ void quantize_q1(const float * x, int32_t * dst) {
    ...
    uint32_t bitset = 0;
    for (int i = 0; i < 32; i++) {
        const uint32_t float_bits = *((const uint32_t*)&x[idx*32 + i]);
        const uint32_t sign_bit = float_bits >> 31;
        bitset |= (sign_bit << i);
    }
    dst[idx] = bitset;
}
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            */
            ...
            FILE *file = fopen("/ssd/workspace/jiho/prosparse-7B-gsm8k-precision-recall.txt", "a");
...
static void ggml_cuda_op_eval_async(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, int layer_id) {
    ...
    const int threads_per_block = WARP_SIZE;
    const int block_num = (src0->ne[0] + threads_per_block - 1) / threads_per_block;
    eval_impl<<< block_num, threads_per_block, 0, compute_stream>>>((float *)src0->data, (float *) dst->data);
}
...
static void ggml_cuda_debug(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // Debug gate Matrix
    cudaStreamSynchronize(stream);
    FILE *file = fopen("/home/ethannlin/Desktop/sparseinfer.cpp/prosparse-13B-Y.txt", "a");

    int il;
    memcpy(&il, dst->op_params, sizeof(int));

    int nrows1 = src1->ne[0];
    int nrows0 = src0->ne[0];

    float *tmp = (float *) malloc(1 * nrows1 * sizeof(float));
    cudaMemcpy(tmp, (float *)src1->data, 1 * nrows1 * sizeof(float), cudaMemcpyDeviceToHost);

    float *tmp2 = (float *) malloc(1 * nrows0 * sizeof(float));
    cudaMemcpy(tmp2, (float *)src0->data, 1 * nrows0 * sizeof(float), cudaMemcpyDeviceToHost);

    fprintf(file, "[ Layer %d: ", il);
    for (int i=0; i<nrows0; i++) {
        fprintf(file, "%f, ", tmp2[i]);
    }
    fprintf(file, "],\n");
    fclose(file);

    debug_impl<<<nrows1 / 32, 32, 0, stream>>>((float *)src1->data, (float *) dst->data);
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    switch (dst->op) {
        ...
        case GGML_OP_EVAL_ASYNC:
            {
                // Unpack layer_id from the extra field
