...
static bool llm_load_tensors(
    ...
    for (uint32_t i=0; i<model.hparams.n_layer; i++) {
        ggml_gate_q1(model.layers[i].ffn_gate, model.layers[i].ffn_gate_q);
...
static struct ggml_tensor * llm_build_ffn(
    ...
    */
    if (cur->ne[1] != 1) {
        ...
        // Separate kernel, utilize h1 sparsity
        // Quantize current input value
        struct ggml_tensor * q_cur = ggml_quantize_q1(ctx, cur);
        // Sparse index predictor

        int scale_factor = alpha ? alpha[il] : 10300;
        ...
        static const int g_contrib = []{
            const char * s = getenv("ACAS_SIGNAL");
            return (s && strcmp(s, "contrib") == 0) ? 1 : 0;
        }();
        ...
        if (cm && do_check && g_contrib && residual) {
            // sparse path
            struct ggml_tensor * s_gate   = ggml_relu(ctx, ggml_mul_mat_vec_sparse_gate(ctx, gate, cur, sparse_idx));
            ...
            struct ggml_tensor * sparse_gate;
            if (cm && do_check) {
                // precision check: dense gate vs the predictor mask
                struct ggml_tensor * full_gate = ggml_mul_mat_vec(ctx, gate, cur);
                struct ggml_tensor * error     = ggml_sparse_error(ctx, full_gate, sparse_idx);
                sparse_gate = ggml_eval_async(ctx, full_gate, error, il);
            } else {
                sparse_gate = ggml_mul_mat_vec_sparse_gate(ctx, gate, cur, sparse_idx);
            }
            sparse_gate = ggml_relu(ctx, sparse_gate);
            struct ggml_tensor * sparse_up = ggml_mul_mat_vec_sparse_up(ctx, up, cur, sparse_idx);
            cur = ggml_mul(ctx, sparse_gate, sparse_up);
...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama(int * alpha, ConfusionMatrix * cm, int * flag) {
        struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, LLAMA_MAX_NODES, false);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cur = llm_build_ffn(ctx0, cur,
                        ...
                        model.layers[il].ffn_gate, NULL, model.layers[il].ffn_gate_q,
...
static struct ggml_cgraph * llama_build_graph(
    ...
    switch (model.arch) {
        case LLM_ARCH_LLAMA:
            {
                result = llm.build_llama(alpha, cm, flag);
            } break;
        case LLM_ARCH_BAICHUAN:
            {
...
static int llama_decode_internal(
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
        ...
        ggml_cgraph * gf = llama_build_graph(lctx, u_batch, false, alpha, cm, flag);
...
static void llama_kv_cache_update_internal(struct llama_context & lctx) {
    ...
    if (need_reserve) {
        ...
        ggml_cgraph * gf = llama_build_graph(lctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true, nullptr, nullptr, nullptr);
    ...
    void fill_next_beams_by_top_probabilities(llama_beam & beam) {
        ...
        if (beam.eob) {
            ...
            if (!beam.tokens.empty()) {
                llama_decode(ctx, llama_batch_get_one(beam.tokens.data(), beam.tokens.size(), n_past, 0), nullptr, nullptr, nullptr);
            }
    ...
    void loop(const llama_beam_search_callback_fn_t callback, void * const callback_data) {
        ...
        for (int i = 0 ; i < n_predict && std::any_of(beams.begin(),beams.end(),not_eob) &&
            ...
            if (common_prefix_length) {
                llama_decode(ctx, llama_batch_get_one(beams[0].tokens.data(), common_prefix_length, n_past, 0), nullptr, nullptr, nullptr);
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        // scheduler and compute buffers
        {
            ...
            ggml_cgraph * gf = llama_build_graph(*ctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true, nullptr, nullptr, nullptr);
...
int32_t llama_decode(
        struct llama_context * ctx,
          ...
          ConfusionMatrix * cm,
          int * flag) {
    const int ret = llama_decode_internal(*ctx, batch, alpha, cm, flag);
    if (ret < 0) {
