...
static results_perplexity perplexity_v2(llama_context * ctx, const gpt_params & params) {
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            if (llama_decode(ctx, llama_batch_get_one(tokens.data() + batch_start, batch_size, j * n_batch, 0), nullptr, nullptr, nullptr)) {
...
static results_perplexity perplexity(llama_context * ctx, const gpt_params & params, const int32_t n_ctx) {
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            if (llama_decode(ctx, batch, nullptr, nullptr, nullptr)) {
                fprintf(stderr, "%s : failed to eval\n", __func__);
                return {tokens, -1, logit_history, prob_history};
            }
...
static bool decode_helper(llama_context * ctx, llama_batch & batch, std::vector<float> & batch_logits, int32_t n_batch, int32_t n_vocab) {
    ...
    for (int32_t i = 0; i < (int32_t) batch.n_tokens; i += n_batch) {
        ...
        const int ret = llama_decode(ctx, batch_view, nullptr, nullptr, nullptr);
        if (ret != 0) {
