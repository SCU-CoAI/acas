...
ggml_tensor * llm_graph_context::build_ffn(
   ...
   llm_ffn_gate_type   type_gate,
                 int   il) const {
    // ACAS dense-baseline activation toggle: force FFN activation so ONE baseline binary
    // measures both dense rows in-frame (ReLU=ProSparse, SiLU=LLaMA). Latency-neutral
    // (matmuls dominate) -- set for correctness. LLAMA_FFN_ACT=relu|silu.
    if (const char * _acas_act = getenv("LLAMA_FFN_ACT")) {
        if      (_acas_act[0]==(char)0x72) type_op = LLM_FFN_RELU;  // r
        else if (_acas_act[0]==(char)0x73) type_op = LLM_FFN_SILU;  // s
    }
