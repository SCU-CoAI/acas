...
struct ggml_tensor * ggml_debug(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int il) {

    const int64_t ne[4] = { b->ne[0], b->ne[1], b->ne[2], b->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op   = GGML_OP_DEBUG;
    result->grad = NULL;
    result->src[0] = a;
    result->src[1] = b;
...
struct ggml_tensor * ggml_fusion_MLP(
        struct ggml_context * ctx,
        struct ggml_tensor  * a, //gate_proj
        struct ggml_tensor  * b, //hidden_sate
        struct ggml_tensor  * c, //up_proj
        struct ggml_tensor  * d, /*sparse_index*/
        int il) {
    GGML_ASSERT(ggml_can_mul_mat(a, b));
    GGML_ASSERT(!ggml_is_transposed(a));
...
struct ggml_tensor * ggml_size_quantize_q1(
    ...
    ggml_set_op_params(result, &std_multiplier, sizeof(std_multiplier));
...
//ggml_sparse_error
struct ggml_tensor * ggml_sparse_error(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {

    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_I32, 4, 1);

    result->op   = GGML_OP_SPARSE_ERROR;
    result->grad = NULL;
    result->src[0] = a;
    result->src[1] = b;
...
//ggml_contrib_error
// a = y_delta (MLP-output drift = down(h_dense - h_sparse)), b = residual stream.
// Result is the same I32[4,1] confusion-style payload as ggml_sparse_error so it can be
// consumed by ggml_eval_async unchanged: the CUDA kernel encodes
//   contrib = clip(1 - ||a|| / ||b||, 0, 1)   ->   TP = round(contrib*S), FP = S-TP, FN=TN=0
// and the async controller precision = TP/(TP+FP) recovers contrib exactly.
...
// Async version that supports layer ID and step for background processing
struct ggml_tensor * ggml_eval_async(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int layer_id) {
    ...
    result->op   = GGML_OP_EVAL_ASYNC;
    result->grad = NULL;
    result->src[0] = a;
    result->src[1] = b;
    
    // Store layer_id in extra field
    int64_t* extra_data = (int64_t*)malloc(sizeof(int64_t));
    *extra_data = (int64_t)layer_id << 32;
    result->extra = extra_data;

    return result;
}

struct ggml_tensor * ggml_mul_mat_vec(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
...
void ggml_make_Q1(struct ggml_tensor * gate, struct ggml_tensor * dst) {
    ggml_half * tmp = (ggml_half *) malloc (sizeof(ggml_half) * gate->ne[0] * gate->ne[1]);
    MemCopy(gate->data, tmp, gate->ne[0] * gate->ne[1] * sizeof(ggml_half));

    int * dst_tmp = (int *) malloc (sizeof(int) * dst->ne[0] * dst->ne[1]);

    for (int i=0; i<gate->ne[1]; i++) {
        for (int col=0; col<dst->ne[0]; col++) {
            unsigned int bitset = 0;
            for (int j=0; j<32; j++) {
                if (isNegative(&tmp[i*gate->ne[0] + j + col*32]))
    ...
    MemCopy_HTOD(dst_tmp, dst->data, dst->ne[0] * dst->ne[1] * sizeof(int));
    free(tmp);
    free(dst_tmp);
}

void ggml_make_size_Q1(struct ggml_tensor * gate, struct ggml_tensor * dst) {
    ggml_half * tmp = (ggml_half *) malloc (sizeof(ggml_half) * gate->ne[0] * gate->ne[1]);
    MemCopy(gate->data, tmp, gate->ne[0] * gate->ne[1] * sizeof(ggml_half));
...
// min 함수: 두 값 중 작은 값을 반환
int min(int a, int b) {
    return (a < b) ? a : b;
}
// 행렬을 타일링을 통해 전치하는 함수
void transpose_tile(const ggml_half * input, ggml_half * output, int width, int height) {
...
void transpose_mat(struct ggml_tensor * down, struct ggml_tensor * dst) {
    ggml_half * tmp = (ggml_half *) malloc (sizeof(ggml_half) * down->ne[0] * down->ne[1]);
    MemCopy(down->data, tmp, down->ne[0] * down->ne[1] * sizeof(ggml_half));

    ggml_half * dst_tmp = (ggml_half *) malloc (sizeof(ggml_half) * dst->ne[0] * dst->ne[1]);

    transpose_tile(tmp, dst_tmp, down->ne[0], down->ne[1]);

    MemCopy_HTOD(dst_tmp, dst->data, dst->ne[0] * dst->ne[1] * sizeof(ggml_half));
    free(tmp);
    free(dst_tmp);
}
