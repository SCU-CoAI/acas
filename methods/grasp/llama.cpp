...
#include "cadence.h"  // ACAS: per-layer MIMD check-cadence gate (g_mimd, defined in sparse_async.cpp)
...
struct llama_layer {
    ...
    struct ggml_tensor * ffn_gate_q;   // 1비트 down 비트 저장소
    struct ggml_tensor * ffn_gate_s;   // 1비트 Size 비트 저장소
    struct ggml_tensor * ffn_down_t;   // down 전치 행렬
...
static bool llm_load_tensors(
            ...
            case LLM_ARCH_MINICPM:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (n_expert == 0) {
                            ...
                            layer.ffn_gate_q = ggml_new_tensor_2d(ctx_split, GGML_TYPE_I32, layer.ffn_gate->ne[0] / 32, layer.ffn_gate->ne[1]);
                            layer.ffn_gate_s = ggml_new_tensor_2d(ctx_split, GGML_TYPE_I32, layer.ffn_gate->ne[0] / 32, layer.ffn_gate->ne[1]);
                            layer.ffn_down = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_DOWN, "weight", i), {  n_ff, n_embd});
                            layer.ffn_down_t = ggml_new_tensor_2d(ctx_split, GGML_TYPE_F16, layer.ffn_down->ne[1], layer.ffn_down->ne[0]);
    ...
    for (uint32_t i=0; i<model.hparams.n_layer; i++) {
        ggml_make_Q1(model.layers[i].ffn_gate, model.layers[i].ffn_gate_q);
        ggml_make_size_Q1(model.layers[i].ffn_gate, model.layers[i].ffn_gate_s);
        transpose_mat(model.layers[i].ffn_down, model.layers[i].ffn_down_t);
        set_constants();
        printf("Layer %d Load Complete\n", i);
    }
...
static struct ggml_tensor * llm_build_ffn(
        struct ggml_context * ctx,
         ...
         const llm_build_cb & cb,
                        int   il,
         struct ggml_tensor * residual = nullptr) {  // ACAS contrib: pre-ffn-norm residual stream
    /*
    */
    if (cur->ne[1] != 1) {
        ...
        //커널 분리, h1 희소도 활용 O
        // X의 이상치들의 합을 저장할 공간 할당
        struct ggml_tensor * x_outlier = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, gate->ne[0]/32);
        // X의 평균, 분산 구하기
        struct ggml_tensor * x_variance = ggml_variance(ctx, cur);
        // 입력값의 사이즈에 따른 비트 정보 계산

        float alpha = alphas ? alphas[il] : 0;
        float val = alpha/10000 - 1;
        const float scale_factor = 0.5f * cur->ne[0] * val;
        // printf("scale_factor: %f\n", scale_factor);
        // printf("layer: %d, scale factor: %f, cur->ne[0]: %d, val: %f, alpha: %f\n", il, scale_factor, cur->ne[0], val, alpha);

        struct ggml_tensor * size_q_cur = ggml_size_quantize_q1(ctx, cur, x_variance, x_outlier, gate, gate_s, 6.0);
        // 현재 입력값 양자화
        ...
        // if (il > 20)
        //     scale_factor = 100;
        struct ggml_tensor * sparse_idx = ggml_mul_mat_vec_q1(ctx, gate_q, q_cur, gate_s, size_q_cur, x_outlier, scale_factor, il);
        // Gate - h1을 상관하지 않을 때는 up 커널 사용
        // ACAS_SIGNAL=contrib selects the contribution check; default is the precision check.
        static const int g_contrib = []{
            const char * e = getenv("ACAS_SIGNAL");
            return (e && strcmp(e, "contrib") == 0) ? 1 : 0;
        }();

        // should_check() advances the per-layer counter; call it exactly once per layer.
        const bool do_check = g_mimd ? g_mimd->should_check(il) : (flag && *flag);

        if (do_check && g_contrib && residual) {
            // sparse path
            struct ggml_tensor * s_gate   = ggml_relu(ctx, ggml_mul_mat_vec_sparse_gate(ctx, gate, cur, sparse_idx, il));
            struct ggml_tensor * s_up     = ggml_mul_mat_vec_sparse_up(ctx, up, cur, sparse_idx);
            struct ggml_tensor * h_sparse = ggml_mul(ctx, s_gate, s_up);

            // dense reference
            struct ggml_tensor * f_gate   = ggml_relu(ctx, ggml_mul_mat_vec(ctx, gate, cur));
            struct ggml_tensor * f_up     = ggml_mul_mat_vec(ctx, up, cur);
            struct ggml_tensor * h_dense  = ggml_mul(ctx, f_gate, f_up);

            // down is linear: subtract first, then a single down GEMV gives y_delta.
            struct ggml_tensor * h_delta  = ggml_sub(ctx, h_dense, h_sparse);
            struct ggml_tensor * y_delta  = ggml_mul_mat_vec(ctx, down, h_delta);

            struct ggml_tensor * cerr     = ggml_contrib_error(ctx, y_delta, residual);

            // eval_async passes the sparse output through and enqueues the payload.
            struct ggml_tensor * y_out    = ggml_mul_mat_vec_transpose(ctx, down_t, h_sparse);
            cur = ggml_eval_async(ctx, y_out, cerr, il);
        } else if (do_check && !g_contrib) {
            // precision check: dense gate vs the predictor mask
            struct ggml_tensor * full_gate = ggml_mul_mat_vec(ctx, gate, cur);
            struct ggml_tensor * perr      = ggml_sparse_error(ctx, full_gate, sparse_idx);
            struct ggml_tensor * p_gate    = ggml_relu(ctx, ggml_mul_mat_vec_sparse_gate(ctx, gate, cur, sparse_idx, il));
            struct ggml_tensor * p_up      = ggml_mul_mat_vec_sparse_up(ctx, up, cur, sparse_idx);
            struct ggml_tensor * p_hidden  = ggml_mul(ctx, p_gate, p_up);
            struct ggml_tensor * p_out     = ggml_mul_mat_vec_transpose(ctx, down_t, p_hidden);
            cur = ggml_eval_async(ctx, p_out, perr, il);
        } else {
            struct ggml_tensor * tmp = ggml_mul_mat_vec_sparse_gate(ctx, gate, cur, sparse_idx, il);
            tmp = ggml_relu(ctx, tmp);
            struct ggml_tensor * tmp2 = ggml_mul_mat_vec_sparse_up(ctx, up, cur, sparse_idx);
            cur = ggml_mul(ctx, tmp, tmp2);
...
struct llm_build_context {
    ...
    struct ggml_tensor * build_x_outlier(struct ggml_context * ctx, int n_cols) {
        struct ggml_tensor * x_outlier = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_cols);
        return x_outlier;
    }

    struct ggml_cgraph * build_llama(float * alphas, int * flag) {
        struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, LLAMA_MAX_NODES, false);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cur = llm_build_ffn(ctx0, cur,
                        ...
                        model.layers[il].ffn_gate, NULL, model.layers[il].ffn_gate_q, model.layers[il].ffn_gate_s,
                        model.layers[il].ffn_down, model.layers[il].ffn_down_t, NULL,
                        NULL,
                        alphas, flag,
                        LLM_FFN_SILU, LLM_FFN_PAR, cb, il,
                        ffn_inp);   // ACAS contrib: residual stream for the drift denominator
                cb(cur, "ffn_out", il);
            } else {
                // MoE branch
                cur = llm_build_norm(ctx0, ffn_inp, hparams,
    ...
    struct ggml_cgraph * build_baichuan() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // feed-forward network
            {
                ...
                cur = llm_build_ffn(ctx0, cur,
                        model.layers[il].ffn_up,   NULL,
                        model.layers[il].ffn_gate, NULL, NULL, NULL,
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // FF
            {
                ...
                cur = llm_build_ffn(ctx0, cur,
                        ...
                        NULL,                      NULL, NULL, NULL,
                        model.layers[il].ffn_down, NULL, model.layers[il].ffn_down_b,
                        NULL,
                        NULL, NULL,
                        LLM_FFN_GELU, LLM_FFN_SEQ, cb, il);
                cb(cur, "ffn_out", il);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // FFN shared expert
            {
                ...
                ggml_tensor * cur_ffn = llm_build_ffn(ctx0, cur,
                        ...
                        model.layers[il].ffn_gate_shexp, NULL, NULL, NULL,
...
static struct ggml_cgraph * llama_build_graph(
    ...
    switch (model.arch) {
        case LLM_ARCH_LLAMA:
            {
                result = llm.build_llama(alphas, flag);
            } break;
        case LLM_ARCH_BAICHUAN:
            {
...
static int llama_decode_internal(
        ...
        float * alphas,
        int * flag) { // TODO: rename back to batch
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
        ...
        ggml_cgraph * gf = llama_build_graph(lctx, u_batch, false, alphas, flag);
...
static void llama_kv_cache_update_internal(struct llama_context & lctx) {
    ...
    if (need_reserve) {
        ...
        ggml_cgraph * gf = llama_build_graph(lctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true, nullptr, nullptr);
    ...
    void fill_next_beams_by_top_probabilities(llama_beam & beam) {
        ...
        if (beam.eob) {
            ...
            if (!beam.tokens.empty()) {
                llama_decode(ctx, llama_batch_get_one(beam.tokens.data(), beam.tokens.size(), n_past, 0), nullptr, nullptr);
            }
    ...
    void loop(const llama_beam_search_callback_fn_t callback, void * const callback_data) {
        ...
        for (int i = 0 ; i < n_predict && std::any_of(beams.begin(),beams.end(),not_eob) &&
            ...
            if (common_prefix_length) {
                llama_decode(ctx, llama_batch_get_one(beams[0].tokens.data(), common_prefix_length, n_past, 0), nullptr, nullptr);
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        // scheduler and compute buffers
        {
            ...
            ggml_cgraph * gf = llama_build_graph(*ctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true, nullptr, nullptr);
...
int32_t llama_decode(
        ...
        float * alphas,
        int * flag) {
    const int ret = llama_decode_internal(*ctx, batch, alphas, flag);
    if (ret < 0) {
...
void llama_synchronize(struct llama_context * ctx) {
    ...
    if (ctx->n_queued_tokens == 1) {
        const int64_t _acas_dt = ggml_time_us() - ctx->t_compute_start_us;
        ctx->t_eval_us += _acas_dt;
        ctx->n_eval++;
        static FILE * _acas_ptok = [](){ const char* p=getenv("ACAS_PTOK_LOG"); FILE* f=p?fopen(p,"w"):nullptr; if(f) fprintf(f,"idx,dt_us\n"); return f; }();
        static long _acas_ptok_idx = 0;
        if (_acas_ptok) fprintf(_acas_ptok, "%ld,%lld\n", _acas_ptok_idx++, (long long)_acas_dt);
