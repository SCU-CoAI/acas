...
int main(int argc, char ** argv) {
    ...
    // evaluate prompt
    llama_decode(ctx, llama_batch_get_one(tokens.data(), tokens.size(), n_past, 0), nullptr, nullptr);
    n_past += tokens.size();
    ...
    for (auto i = 0; i < params.n_predict; i++) {
        ...
        if (llama_decode(ctx, llama_batch_get_one(&next_token, 1, n_past, 0), nullptr, nullptr))
        {
    ...
    for (auto i = 0; i < params.n_predict; i++) {
        ...
        if (llama_decode(ctx2, llama_batch_get_one(&next_token, 1, n_past, 0), nullptr, nullptr))
        {
            fprintf(stderr, "\n%s : failed to evaluate\n", __func__);
            llama_free(ctx2);
            llama_free_model(model);
    ...
    for (auto i = 0; i < params.n_predict; i++) {
        ...
        if (llama_decode(ctx3, llama_batch_get_one(&next_token, 1, n_past, 1), nullptr, nullptr))
        {
            fprintf(stderr, "\n%s : failed to evaluate\n", __func__);
            llama_free(ctx3);
            llama_free_model(model);
