...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    max_n_tensors += n_layer*1; // transposed tensor for down (AXPY pattern)
            ...
            case LLM_ARCH_GRANITE_MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (n_expert == 0) {
                            ...
                            // Create tensors with same buffer type as ffn_gate/ffn_down
                            ggml_backend_buffer_type_t buft = pimpl->dev_layer.at(i).buft_list->front().second;
                            ggml_context * layer_ctx = ctx_for_buft(buft);
                            
                            // threshold prediction
                            layer.ffn_gate_q = ggml_new_tensor_2d(layer_ctx, GGML_TYPE_I32, layer.ffn_gate->ne[0] / 32, layer.ffn_gate->ne[1]);
                            layer.ffn_gate_size = ggml_new_tensor_2d(layer_ctx, GGML_TYPE_I32, layer.ffn_gate->ne[0] / 32, layer.ffn_gate->ne[1]);
                            layer.ffn_down_t = ggml_new_tensor_2d(layer_ctx, GGML_TYPE_F16, layer.ffn_down->ne[1], layer.ffn_down->ne[0]);
    ...
    // Generate derived tensors from loaded tensors (tensors already created during model loading)
    for (uint32_t i = 0; i < hparams.n_layer; i++) {
        // gate quantization
        if (layers[i].ffn_gate_q && layers[i].ffn_gate) {
            ggml_gate_q1(layers[i].ffn_gate, layers[i].ffn_gate_q);
        }

        // gate size
        if (layers[i].ffn_gate_size && layers[i].ffn_gate) {
            ggml_gate_size_Q1(layers[i].ffn_gate, layers[i].ffn_gate_size);
        }
        
        // Transpose down weights for AXPY pattern
        if (layers[i].ffn_down_t && layers[i].ffn_down) {
            transpose_mat(layers[i].ffn_down, layers[i].ffn_down_t);
        }
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cur = build_ffn(cur,
                        ...
                        model.layers[il].ffn_gate_q, model.layers[il].ffn_gate_size, model.layers[il].ffn_down_t,
