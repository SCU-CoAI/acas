...
static void transpose_tile(const ggml_half * input, ggml_half * output, int width, int height);
#include <stdarg.h>
#include <signal.h>
...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "up_gated(x)",
    "contrib_error(x,y)",
    "predictor_skip_mask(h,m)",
    "contrib_error_predictor(x,y)",
};
...
//ggml_quantize_q1
struct ggml_tensor * ggml_quantize_q1(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {

    const int64_t ne[4] = { a->ne[0] / 32, a->ne[1], a->ne[2], a->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_I32, 4, ne);

    result->op   = GGML_OP_QUANTIZE_Q1;
    result->src[0] = a;
...
//ggml_variance
struct ggml_tensor * ggml_variance(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {

    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 64);

    result->op   = GGML_OP_VARIANCE;
    result->src[0] = a;

    return result;
}

struct ggml_tensor * ggml_size_quantize_q1(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * x_variance,
        struct ggml_tensor  * x_outlier) {

    const int64_t ne[4] = { a->ne[0] / 32, a->ne[1], a->ne[2], a->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_I32, 4, ne);

    result->op   = GGML_OP_SIZE_QUANTIZE_Q1;
    result->src[0] = a;
    result->src[1] = x_variance;
    result->src[2] = x_outlier;

    return result;
}

//ggml_mul_mat_vec_q1
struct ggml_tensor * ggml_mul_mat_vec_q1(
        struct ggml_context * ctx,
        struct ggml_tensor  * a, //gate_q
        struct ggml_tensor  * b, //q_cur
        struct ggml_tensor  * c, //gate_s
        struct ggml_tensor  * d, //size_q_cur,
        struct ggml_tensor  * e, //x_outlier
        int scale_factor,
        int il) {

    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_I32, a->ne[1], b->ne[1]);

    int param[2] = {scale_factor, il};
    ggml_set_op_params(result, param, sizeof(param));
...
//ggml_mul_mat_vec_sparse_gate
struct ggml_tensor * ggml_mul_mat_vec_sparse_gate(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        struct ggml_tensor  * c) {

    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, a->ne[1], b->ne[1]);

    result->op   = GGML_OP_MUL_MAT_VEC_S_G;
    result->src[0] = a;
    result->src[1] = b;
    result->src[2] = c;
...
//ggml_mul_mat_vec_sparse_up
struct ggml_tensor * ggml_mul_mat_vec_sparse_up(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        struct ggml_tensor  * c) {
    ...
    result->op   = GGML_OP_MUL_MAT_VEC_S_U;
    result->src[0] = a;
    result->src[1] = b;
    result->src[2] = c;
...
//ggml_mul_mat_vec_transpose
struct ggml_tensor * ggml_mul_mat_vec_transpose(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {

    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, a->ne[0], b->ne[1]);

    result->op   = GGML_OP_MUL_MAT_VEC_T;
    result->src[0] = a;
    result->src[1] = b;
...
// 2D Adaptive Sparsity Operations Implementation

struct ggml_tensor * ggml_threshold_input(
        struct ggml_context * ctx,
        struct ggml_tensor  * input,
        float threshold,
        int layer_idx,
        bool compute_quality) {
    // Creates a mask for column-wise sparsity based on input magnitude  
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, input->ne[0]);
    
    result->op = GGML_OP_THRESHOLD_INPUT;
    result->src[0] = input;
    
    // Store threshold, layer_idx and compute_quality in op_params
    ggml_set_op_params_f32(result, 0, threshold);
    ggml_set_op_params_i32(result, 1, layer_idx);
    ggml_set_op_params_i32(result, 2, compute_quality ? 1 : 0);
    
    return result;
}

struct ggml_tensor * ggml_sparse_gate_2d(
        struct ggml_context * ctx,
        struct ggml_tensor  * gate,
        struct ggml_tensor  * masked_input,
        float activation_threshold,
        int layer_idx,
        bool compute_quality) {
    // Gate operation with row sparsity from activation threshold
    // masked_input already has zeros where input threshold not met
    // gate is NON-TRANSPOSED: [n_embd, n_ff], masked_input is [n_embd], output is [n_ff]
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, gate->ne[1]);
    
    result->op = GGML_OP_SPARSE_GATE_2D;
    result->src[0] = gate;
    result->src[1] = masked_input;
    
    // Store activation threshold, layer_idx and compute_quality in op_params
    ggml_set_op_params_f32(result, 0, activation_threshold);
    ggml_set_op_params_i32(result, 1, layer_idx);
    ggml_set_op_params_i32(result, 2, compute_quality ? 1 : 0);
...
struct ggml_tensor * ggml_sparse_up_2d(
        struct ggml_context * ctx,
        struct ggml_tensor  * up,
        struct ggml_tensor  * masked_input,
        struct ggml_tensor  * row_mask,
        int layer_idx) {
    // Up operation with row sparsity from gate mask
    // masked_input already has zeros where input threshold not met
    // up is [n_embd, n_ff], masked_input is [n_embd], output is [n_ff]
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, up->ne[1]);
    
    result->op = GGML_OP_SPARSE_UP_2D;
    result->src[0] = up;
    result->src[1] = masked_input;
    result->src[2] = row_mask;  // Row mask from gate
    
    // Store layer_idx in op_params
    ggml_set_op_params_i32(result, 0, layer_idx);
    
    return result;
}

struct ggml_tensor * ggml_sparse_gate_up_2d(
        struct ggml_context * ctx,
        struct ggml_tensor  * gate,
        struct ggml_tensor  * up,
        struct ggml_tensor  * masked_input,
        ...
        int layer_idx,
        bool compute_quality) {
    // Fused Gate (with activation threshold) and Up projections in one pass
    // gate, up: [n_in, n_out] (non-transposed), masked_input: [n_in], output: [n_out]
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, gate->ne[1]);

    result->op = GGML_OP_SPARSE_GATE_UP_2D;
    result->src[0] = gate;
    result->src[1] = up;
    result->src[2] = masked_input;
...
struct ggml_tensor * ggml_sparse_down(
        struct ggml_context * ctx,
        struct ggml_tensor  * down_t,
        struct ggml_tensor  * input,
        int layer_idx) {
    // Down operation with row sparsity
    // down_t is transposed: [n_embd, n_ff], input is [n_ff] with zeros, output is [n_embd]
    // Input already has zeros from sparse_up output - no separate mask needed
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, down_t->ne[0]);
    
    result->op = GGML_OP_SPARSE_DOWN;
    result->src[0] = down_t;
    result->src[1] = input;
...
// audit-post op: compare thresholded teacher activation vs predictor mask and accumulate metrics
struct ggml_tensor * ggml_audit_post(
        struct ggml_context * ctx,
        struct ggml_tensor  * teacher_masked,
        struct ggml_tensor  * pred_mask,
        int   layer_idx,
        bool  compute_quality) {
    struct ggml_tensor * result = ggml_view_tensor(ctx, teacher_masked);
    result->op = GGML_OP_AUDIT_POST;
    result->src[0] = teacher_masked;
    result->src[1] = pred_mask;
    // Store layer_idx and compute_quality
    ggml_set_op_params_i32(result, 0, layer_idx);
    ggml_set_op_params_i32(result, 1, compute_quality ? 1 : 0);
    return result;
}

// Dense-derived Up with row gating: out = gate ⊙ (Up·x [+b] [×s])
struct ggml_tensor * ggml_up_gated(
        struct ggml_context * ctx,
        struct ggml_tensor  * up,
    ...
    GGML_ASSERT(up->ne[1] == gate->ne[0]);
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, up->ne[1]);
    result->op = GGML_OP_UP_GATED;
    result->src[0] = up;
    result->src[1] = x;
    result->src[2] = gate;
    result->src[3] = up_b;
...
// a = y_delta (MLP-output drift = down(h_dense - h_sparse), one dense down GEMV -- mathematically
// identical to down(h_dense)-down(h_sparse) since down is linear), b = residual stream (ffn_inp).
// The CUDA kernel reduces ||a|| and ||b|| and writes [||a||, ||b||] into the layer's
// activation-quality slot so the existing controller recovers quality = 1 - ||a||/||b|| == contrib.
struct ggml_tensor * ggml_contrib_error(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int layer_idx) {
    // Result is a tiny passthrough handle; the real output is the write into the GPU quality buffer.
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 2);

    result->op = GGML_OP_CONTRIB_ERROR;
    result->src[0] = a;
    result->src[1] = b;
...
// ACAS predictor-knob masking: out[row] = (pred_mask[row]==0) ? h_dense[row] : 0.
// Keeps ONLY the predictor-skipped rows (the complement of the forward predictor keep set), so the
// result is exactly the predictor-skipped neurons' intermediate contribution h_pred_delta.
struct ggml_tensor * ggml_predictor_skip_mask(
        struct ggml_context * ctx,
        struct ggml_tensor  * h_dense,
        struct ggml_tensor  * pred_mask) {
    GGML_ASSERT(h_dense->ne[0] == pred_mask->ne[0]);
    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, h_dense->ne[0]);
    result->op = GGML_OP_PREDICTOR_SKIP_MASK;
    result->src[0] = h_dense;
    result->src[1] = pred_mask;
    return result;
}

// ACAS predictor-knob output-contribution drift. Same reduction as ggml_contrib_error, but the CUDA
// impl writes [||a||, ||b||] into the layer's PREDICTOR slot and rides the predictor async transfer
// (is_activation=false), so the predictor controller is driven by this drift in contrib mode.
struct ggml_tensor * ggml_contrib_error_predictor(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int layer_idx) {
    ...
    result->op = GGML_OP_CONTRIB_ERROR_PREDICTOR;
    result->src[0] = a;
    result->src[1] = b;
...
// Alias for clarity: gate path uses the same op id; mask can be int or float
struct ggml_tensor * ggml_gate_gated(
        struct ggml_context * ctx,
    ...
    return ggml_up_gated(ctx, gate_w, x, mask, gate_b, gate_s);
}
...
// =====================================> sparseinfer
// gate q1 quantization (extracting sign bits of the gate tensor)
void ggml_gate_q1(
        struct ggml_tensor * gate,
        struct ggml_tensor * dst) {

    size_t gate_size = sizeof(ggml_half) * gate->ne[0] * gate->ne[1];
    ggml_half * tmp = (ggml_half *) malloc(gate_size);
    
    // Use backend tensor get to copy from GPU to CPU
    ggml_backend_tensor_get(gate, tmp, 0, gate_size);
    
    size_t dst_size = sizeof(int) * dst->ne[0] * dst->ne[1];
    int * dst_tmp = (int *) malloc(dst_size);

    for (int row=0; row < gate->ne[1]; row++) {
        for (int col=0; col < dst->ne[0]; col++) {
            unsigned int bitset = 0;
            
            #pragma unroll
            for (int i=0; i<32; i++) {
                unsigned int sign = tmp[row * gate->ne[0] + col * 32 + i] >> 15; // each element in gate is FP16 (extracting sign bit)
                bitset |= (sign << i);
            }
            dst_tmp[row * dst->ne[0] + col] = bitset;
        }
    }

    // Use backend tensor set to copy from CPU to GPU
    ggml_backend_tensor_set(dst, dst_tmp, 0, dst_size);

    free(tmp);
    free(dst_tmp);
}

void ggml_gate_size_Q1(struct ggml_tensor * gate, struct ggml_tensor * dst) {
    size_t gate_size = sizeof(ggml_half) * gate->ne[0] * gate->ne[1];
    ggml_half * tmp = (ggml_half *) malloc(gate_size);
    ...
    float * Q1Q3 = (float *) malloc (sizeof(float) * gate->ne[1] * 2);
    size_t dst_size = sizeof(int) * dst->ne[0] * dst->ne[1];
    int * dst_tmp = (int *) malloc(dst_size);

    calQ1Q3(tmp, Q1Q3, gate->ne[1], gate->ne[0]);

    for (int i=0; i<gate->ne[1]; i++) {
        for (int col=0; col<dst->ne[0]; col++) {
            unsigned int bitset = 0;
            for (int j=0; j<32; j++) {
                if (isBig(Q1Q3[i*2 + 0], Q1Q3[i*2 + 1], &tmp[i*gate->ne[0] + j + col*32]))
                    bitset |= ((unsigned int)1 << j);
            }
            dst_tmp[i*dst->ne[0] + col] = bitset;
        }
    }
...
// Function to transpose a matrix using tiling
static void transpose_tile(const ggml_half * input, ggml_half * output, int width, int height) {
    const int TILE_SIZE = 32;
    for (int row_tile = 0; row_tile < height; row_tile += TILE_SIZE) {
        for (int col_tile = 0; col_tile < width; col_tile += TILE_SIZE) {
            // Iterate within each tile
            for (int i = row_tile; i < min(row_tile + TILE_SIZE, height); ++i) {
                for (int j = col_tile; j < min(col_tile + TILE_SIZE, width); ++j) {
...
void transpose_mat(struct ggml_tensor * down, struct ggml_tensor * dst) {
    size_t down_size = sizeof(ggml_half) * down->ne[0] * down->ne[1];
    ggml_half * tmp = (ggml_half *) malloc(down_size);
    
    // Use backend tensor get to copy from GPU to CPU
    ggml_backend_tensor_get(down, tmp, 0, down_size);

    size_t dst_size = sizeof(ggml_half) * dst->ne[0] * dst->ne[1];
    ggml_half * dst_tmp = (ggml_half *) malloc(dst_size);

    transpose_tile(tmp, dst_tmp, down->ne[0], down->ne[1]);
...
void set_constants() {
    ggml_cuda_set_device_constants();
}
